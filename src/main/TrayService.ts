import { Menu, MenuItem, MenuItemConstructorOptions, nativeImage, Tray } from "electron";
    ...
    this.monerodInstance.monerodLatestData$.pipe(distinctUntilChanged())
      .subscribe((state) => {
        const updatedState = {
          ...
          syncPercentage: this.monerodInstance.calcSyncPercentage(state.height, state.targetHeight)
        }
