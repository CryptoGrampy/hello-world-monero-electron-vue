import { Menu, MenuItem, nativeImage, Tray } from "electron";
import { distinctUntilChanged, Observable, Subject } from "rxjs";
import { Monerod } from "./monerod";

enum StatusLabels {
  ONLINE = 'Monero Node: Online',
  OFFLINE = 'Monero Node: Offline',
  SYNCING = 'Monero Node: Syncing',
  ERROR = 'Monero Node: Not Detected',
}

// Data needed in Tray.. probably be moved into Tray class
export interface TrayData {
  isBusySyncing: boolean,
  databaseSize: number,
  isOffline: boolean,
  updateAvailable: boolean,
  isSynchronized: boolean,
  numOutgoingConnections: number,
  numIncomingConnections: number,
  numRpcConnections: number,
}


export class TrayManager {
  private readonly TRAY_ICON = nativeImage.createFromDataURL('data:image/png;base64,')
  private readonly tray = new Tray(this.TRAY_ICON)
  private readonly trayDataSubject: Subject<TrayData> = new Subject();

  public get trayData$(): Observable<TrayData> {
    return this.trayDataSubject.asObservable()
  }

  // TODO: Clean up... Currently using this as default menu items
  private menuItems = [
    { id: 'status', label: StatusLabels.ERROR, type: 'normal', enabled: false },
    { id: 'sync', label: `Sync: 0%`, type: 'normal', enabled: false },
    { id: 'freeSpace', label: `Free Storage Remaining: 0%`, type: 'normal', enabled: false },
    { id: 'connections', label: 'Connections // Out: 0 P2P: 0 RPC: 0', type: 'normal', enabled: false },
    { label: 'Sep1', type: 'separator' },
    { label: 'Dashboard', type: 'normal' },
    { label: 'Sep2', type: 'separator' },
    { label: 'Stop Node', type: 'normal' },
    { label: 'Restart Node', type: 'normal' },
    { label: 'Autostart on Boot', type: 'checkbox' },
    { label: 'Sep3', type: 'separator' },
    { label: 'Settings', type: 'normal' },
    { label: 'Sep4', type: 'separator' },
    { label: 'Quit', type: 'normal', role: 'quit'},
  ]

  constructor(private readonly moneroD: Monerod) {
    // Subscribe to Monerod state change, convert to necessary data for Menu
    this.moneroD.state$.pipe(distinctUntilChanged())
      .subscribe((state) => {
        // console.log(state)
        // console.log(!this.test)
        // this.menuItems[2].label = String(!this.test)
        // this.test = !this.test
        this.trayDataSubject.next({
          isBusySyncing: state.isBusySyncing,
          databaseSize: state.databaseSize,
          isOffline: state.isOffline,
          updateAvailable: state.updateAvailable,
          isSynchronized: state.isSynchronized,
          numOutgoingConnections: state.numOutgoingConnections,
          numIncomingConnections: state.numIncomingConnections,
          numRpcConnections: state.numRpcConnections,
        })

        this.trayDataSubject.pipe(distinctUntilChanged((a, b) => JSON.stringify(a) === JSON.stringify(b))).subscribe((trayData) => {
          this.updateMenuItems(trayData)
          this.generateTray()
        })

        this.generateTray()
      })

  }

  // TODO: Unsubscribe :)
  private updateMenuItems(trayData: TrayData) {
    this.menuItems = [ 
    { id: 'status', label: this.updateStatus(trayData), type: 'normal', enabled: false },
    { id: 'sync', label: `Sync: ${trayData.isSynchronized ? 100 : 0}%`, type: 'normal', enabled: false },
    // { id: 'freeSpace', label: `Free Storage Remaining: 0%`, type: 'normal', enabled: false },
    { id: 'connections', label: `Connections: Out: ${trayData.numOutgoingConnections} P2P: ${trayData.numIncomingConnections} RPC: ${trayData.numRpcConnections}`, type: 'normal', enabled: false },
    { label: 'Sep1', type: 'separator' },
    { label: 'Dashboard', type: 'normal' },
    { label: 'Sep2', type: 'separator' },
    { label: 'Stop Node', type: 'normal' },
    { label: 'Restart Node', type: 'normal' },
    { label: 'Autostart on Boot', type: 'checkbox' },
    { label: 'Sep3', type: 'separator' },
    { label: 'Settings', type: 'normal' },
    { label: 'Sep4', type: 'separator' },
    { label: 'Quit', type: 'normal', role: 'quit'}
  ]
}

  private updateStatus(trayData: TrayData): string {
    if (!trayData.isOffline && !trayData.isBusySyncing) {
      return StatusLabels.ONLINE
    } else if (trayData.isBusySyncing) {
      return StatusLabels.SYNCING
    } else if (trayData.isOffline) {
      return StatusLabels.OFFLINE
    } else {
      return StatusLabels.ERROR
    }
  }

  private generateTray() {
    this.tray.setContextMenu(Menu.buildFromTemplate(this.menuItems as MenuItem[]))
  }
}